...
function json2db(){
    ...
    outbox_json.orderedItems.forEach((val,idx)=>{
        ...
        val.id=val.id.replace(/users\/.*\/statuses/,"web/statuses").replace("/activity","");
...
function search(input){
    ...
    let keys=[];
    if(id=="likes"){
        keys=Object.keys(likes).filter(key=>likes[key].likeacct==value);
        if(keys.length===0){
            elem("request").innerText="No like was found for "+value+".";
            elem("result").innerText="";
        }else{
            elem("request").innerText="There are "+keys.length+" results for "+value+".";
            elem("result").innerHTML="<table>"+keys.reverse().map(key=>{
                return [
                    "<tr><th class=searchright><a target='_blank' href=",
                    key.replace(/users\/.*\/statuses/,"web/statuses"),
                    ">",
                    key,
                    "</a></th></tr>"
                ].join("");
        ...
        if(id=="word"){
            keys=published.posts.filter(key=>outbox[key].content.includes(value));
        }else if(id=="boost"){
            if(published.reblog[value]){
                keys=published.reblog[value];
            }
        }else if(id=="reply"){
            let replylist={};
            if(published.mentions[value]){
                published.mentions[value].forEach(key=>{
                    replylist[outbox[key].inReplyTo.substr(outbox[key].inReplyTo.lastIndexOf("/"),4)]=key;
                });
            }
            keys=Object.keys(replylist).map(key=>replylist[key]);
        }else if(id=="date"){
            keys=Object.keys(outbox).filter(key=>key.includes(value));
        }
        if(keys.length===0){
            elem("request").innerText="No toot was found for "+value+".";
            elem("result").innerText="";
        }else{
            elem("request").innerText="There are "+keys.length+" results for "+value+".";
            elem("result").innerHTML="<table>"+keys.sort().reverse().map(key=>{
                return [
                    "<tr><th class=searchleft><a target='_blank' href=",
                    outbox[key].id,
                    ">",
                    key.substr(0,19),
                    "</a></th><th class=searchright>",
                    outbox[key].content,
                    "</th></tr>"
                ].join("");
            }).join("")+"</table>";
